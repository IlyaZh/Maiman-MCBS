...
ParserWorker::ParserWorker(const QString& fileName, ParserType type,
                           QObject* parent)
    : QObject(parent),
...
TreeItem* ParserWorker::data() { return m_data; }
    ...
    if (file->open(QIODevice::ReadOnly)) {
      ...
      if (m_parser) {
        if (m_parser->start()) {
          ...
          QString errorString =
              QString("Parsing erorr in file %1! \"%2\"")
                  .arg(file->fileName(), m_parser->errorString());
      ...
      QString errorString =
          QString("File %1 can't be open!").arg(file->fileName());
      emit errorOccured(errorString);
    }
  } else {
