...
struct DeviceDelays {
  ...
  DeviceDelays(quint16 stopDelayMs = COM_STOP_DELAY_MS,
               ...
               quint16 timeout_ms = Const::kComPortTimeout)
...
struct DeviceDescription {
  DeviceDescription(const QPixmap& pic, const QString& desc,
                    const QString& link)
      : Photo(pic), Description(desc), Link(link) {}
  QPixmap Photo;
  QString Description;
  QString Link;
};
...
struct DeviceModel {
  DeviceModel();
  DeviceModel(quint16 id, const QString& name, const DeviceDelays& delays,
              const QMap<quint16, CommandSettings>& cmdBuilders);
  quint16 id;
  QString name;
  DeviceDelays delays;
