...
#if defined(QT_DEBUG)
const QString UpdatesChecker::m_updaterFile =
    "C:/Program Files/Maiman Electronics/Multi Workbench/MaintenanceTool.exe";
#else
...
QString UpdatesChecker::version() const { return m_version; }
qint64 UpdatesChecker::size() const { return m_size; }
...
void UpdatesChecker::checkForUpdates() {
  ...
  connect(m_maintanceProcess, &QProcess::errorOccurred, this,
          [this](QProcess::ProcessError error) {
            Q_UNUSED(error)
            emit errorOccured(m_maintanceProcess->errorString());
          });
  connect(m_maintanceProcess,
          QOverload<int, QProcess::ExitStatus>::of(&QProcess::finished), this,
          &UpdatesChecker::analyzeOutput);
...
void UpdatesChecker::analyzeOutput() {
  auto output =
      QString(m_maintanceProcess->readAllStandardOutput()).split('\n');
  ...
  if (!xml.isEmpty()) {
    ...
    while (!xmlStream->atEnd() && !xmlStream->hasError()) {
      ...
      if (token == QXmlStreamReader::StartElement && !xmlStream->hasError()) {
