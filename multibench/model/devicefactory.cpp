...
void DeviceFactory::start() {
  ...
  connect(m_parseWorker.get(), SIGNAL(finished()), this,
          SLOT(parsingFinished()));

  connect(m_thread, SIGNAL(finished()), m_thread, SLOT(deleteLater()));
  connect(m_parseWorker.get(), SIGNAL(errorOccured(QString)), this,
          SLOT(threadError(QString)));
  connect(m_parseWorker.get(), SIGNAL(errorOccured(QString)), m_thread,
          SLOT(quit()));
  connect(m_parseWorker.get(), SIGNAL(errorOccured(QString)), m_thread,
          SLOT(deleteLater()));
  m_thread->start();
...
QSharedPointer<Device> DeviceFactory::createDevice(quint8 addr, quint16 id) {
  ...
  if (devModel.id == 0) {
    qWarning() << "Device model with id =" << id << "has not found in config!";
    return QSharedPointer<Device>();
  }
  return QSharedPointer<Device>::create(addr, devModel, this);
}

QStringList DeviceFactory::getBaudrate() { return m_baudrates; }
...
void DeviceFactory::parsingFinished() {
  ...
  if (parseTree(*parserTree)) {
    qInfo() << "Parsing config is end succesfully";
...
QMultiMap<uint, QString> DeviceFactory::parseCommonDevId(const TreeItem& item) {
  ...
  for (int i = 0; i < item.childCount(); i++) {
    ...
    if (devId != 0) list.insert(devId, devName);
  }
...
const DeviceModel DeviceFactory::parseDevice(const TreeItem& item) {
  ...
  if (hasCommands && hasId && hasName) {
    DeviceDelays delays =
        DeviceDelays(stopDelayMs, minCommandDelayMs, maxCommandDelayMs);
...
const QMap<quint16, CommandSettings> DeviceFactory::parseCommands(
    const TreeItem& item) {
  ...
  for (int c = 0; c < item.childCount(); c++) {
    ...
    for (int i = 0; i < cmd.childCount(); i++) {
      ...
      if (child.name() == "divider") divider = child.value().toDouble();

      if (child.name() == "tol") tol = child.value().toUInt();

      if (child.name() == "interval") interval = child.value().toUInt();
