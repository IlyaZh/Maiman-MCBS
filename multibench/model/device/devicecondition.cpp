...
DeviceCondition::DeviceCondition(
    ...
    const QVector<Led>& Leds, QLabel* label, QObject* parent)
    : QObject(parent), m_commands(commands), m_Leds(Leds), m_Label(label) {
  for (int idleds = 0; idleds < m_Leds.count(); ++idleds) {
    ...
    for (int idled = 0; idled < led.ledMasks.count(); ++idled) {
      ...
      connect(
          cmd.get(), &DevCommand::updatedValue, this,
          [this, cmd, ledmask]() { setStateText(cmd->valueInt(), ledmask); });
    }
  }
}
