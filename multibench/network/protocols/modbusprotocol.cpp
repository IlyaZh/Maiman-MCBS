...
quint16 ModbusProtocol::calcCrc(const QByteArray& byteArray) {
  ...
  while (i < byteArray.size()) {
    ...
    RetCRC =
        static_cast<quint16>(((loBYTE(RetCRC) ^ CRC_HTable.at(index)) << 8) |
                             (CRC_LTable.at(index)));
    i++;
  }
  return RetCRC;
...
QByteArray ModbusProtocol::setDataValue(quint8 addr, quint16 reg,
                                        quint16 value) {
...
QByteArray ModbusProtocol::getDataValue(quint8 addr, quint16 reg,
                                        quint8 count) {
  QByteArray package = QByteArray();
  package.append(static_cast<quint8>(addr));
...
ModbusProtocol::DataVector ModbusProtocol::execute(
    const QByteArray& rxPackage, const QByteArray& lastTxPackage) {
  m_error = false;
  m_errorString.clear();
        ...
        case Read:
          ...
          if (rxPackage.size() == 5 + bytesCount) {
            if (lastTxPackage.size() >= 4) {
              quint16 lastWriteReg =
                  ((static_cast<quint16>(lastTxPackage.at(2)) << 8) & 0xff00) +
                  static_cast<quint8>(lastTxPackage.at(3));
              for (quint16 i = 0; i < bytesCount / 2; ++i) {
                reg = lastWriteReg + i;
                value = static_cast<quint16>(
                    ((static_cast<quint16>(rxPackage.at(3 + 2 * i)) << 8) &
                     0xff00) +
                    static_cast<quint8>(rxPackage.at(4 + 2 * i)));
          ...
          break;
        case WriteOne:
          reg = static_cast<quint16>(
              (static_cast<quint16>(rxPackage.at(2)) << 8) +
              static_cast<quint16>(rxPackage.at(3)));
          value = static_cast<quint16>(
              ((static_cast<quint16>(rxPackage.at(4)) << 8) & 0xff00) +
              static_cast<quint8>(rxPackage.at(5)));
          resultVector.append({addr, reg, value});
          break;
        default:
          qWarning()
              << QString("[%1] Wrong command %2").arg("Modbus").arg(comm);
      }
  ...
  if (package.size() >= 6) {
    ...
    switch (cmd) {
      case WriteOne:
        ...
        break;
      case Read:
        count = (static_cast<quint8>(package.at(4)) << 8) +
                static_cast<quint8>(package.at(5));
        return 5 + 2 * count;
        break;
    }
  }
...
const QVector<quint8> ModbusProtocol::CRC_HTable{
...
const QVector<quint8> ModbusProtocol::CRC_LTable{
