...
struct SoftProtocolData {
  ...
  bool operator==(const SoftProtocolData& other) const {
    return ((addr == other.addr) && (reg == other.reg) &&
            (value == other.value));
  }
};
...
class SoftProtocol {
 public:
  ...
  virtual QByteArray getDataValue(quint8 addr, quint16 reg,
                                  quint8 count = 1) = 0;
  virtual DataVector execute(const QByteArray& rxPackage,
                             const QByteArray& lastTxPackage) = 0;
