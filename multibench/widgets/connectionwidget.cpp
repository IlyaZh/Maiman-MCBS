...
ConnectionWidget::ConnectionWidget(QWidget* parent)
  ...
  connect(ui->networkConnectButton, &QPushButton::clicked, this,
          &ConnectionWidget::connectToTCP);
  connect(ui->connectComPortButton, &QPushButton::clicked, this,
          &ConnectionWidget::connectToCOM);
  connect(ui->refreshComPortButton, &QPushButton::clicked, this,
          &ConnectionWidget::refreshComPorts);

  connect(ui->comPortComboBox, &QComboBox::currentTextChanged, this,
...
void ConnectionWidget::setConnected(bool isConnected) {
  qInfo() << "Change connection status by user:" << isConnected;
  if (isConnected) {
...
void ConnectionWidget::setPortList(const QStringList& portList) {
  ...
  if (portList.contains(current)) ui->comPortComboBox->setCurrentText(current);
}

void ConnectionWidget::setCurrentComPort(QString port) {
  ui->comPortComboBox->setCurrentText(port);
  qInfo() << "Change com-port by user's selector:"
...
void ConnectionWidget::setProtocol(Const::PortType /*type*/) {
