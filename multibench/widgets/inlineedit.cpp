...
const QString readOnly = "QLineEdit{background-color: rgb(51, 51, 51);\
    color: rgb(255,255,255);\
    border-color: rgb(51, 51, 51);\
    border: 1px;\
    font: 20pt Share Tech Mono;}";

const QString editOnly = "QLineEdit{background-color: rgb(26, 26, 26);\
    color: rgb(255,255,255);\
    ...
    m_layout(new QHBoxLayout)
{
    ...
    m_icon->setStyleSheet(QString::fromUtf8("border: 2px solid rgb(26,26,26);\n"
                                               "border-radius: 3px;\n"
    ...
    connect(this, &InLineEdit::editingFinished, this, &InLineEdit::finishedChanges);
...
void InLineEdit::checkTextLenght(){
    if(!QLineEdit::text().contains("ID:")){
        m_name = QLineEdit::text();
        if(m_name.size() + lenghtNonEditableString + QString::number(m_address).size()>maxLengthString){
            qDebug()<<"+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"<<m_name.size();

            m_name = m_name.remove(maxLengthString - lenghtNonEditableString - QString::number(m_address).size(),m_name.size());
            QLineEdit::setText(m_name);
        }
    }
}
